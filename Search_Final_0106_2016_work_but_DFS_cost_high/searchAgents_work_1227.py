...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        if self.reset_startState == True:
            ...
            print "start state reset to corner state:" ,self.corner_state
    ...
    def getSuccessors(self, state):
        ...
        if state in self.corners and state not in self.visited_corner:
            ...
            if len(self.visited_corner) == len(self.corners):
                ...
                self.reset_startState = False
                # all 4 corners are visisted, set Goal to True
                #if self.visited_corner == self.corners:
                    #Goal = True
        # ignore visisted corner ( after visited_state_list set to 0, corner will be accssed multi-time)
        #aviod corner state visited multiple-times after clear visited state list, this should be out of action loop
        #if state not in self.visited_corner:
            #self.reset_startState = False
...
#idea: 1) check start point to corner (as Goal) distance, real disstance ,not Manhanttan, return longest action corner
    ...
    while len(temp_corners) > 1:
        ...
        for corner in temp_corners:
            if (abs(xy1[0] - corner[0]) + abs(xy1[1] - corner[1])) <= min_distance:
                min_distance = abs(xy1[0] - corner[0]) + abs(xy1[1] - corner[1])
...
def foodHeuristic(state, problem):
    ...
    #print "Food count", state[1].count()

    #"The Min Manhattan distance heuristic to foodgrid

    #foodgrid is a list of (x,y) which has food in space

    if len(foodGrid.asList()) == 0:  # Goal case, skip below

        return 0

    #has food case

    min_distance = min((abs(xy1[0] - grid[0]) + abs(xy1[1] - grid[1])) for grid in foodGrid.asList())
